        ...
        <div>
            ...
            <div className="grid grid-cols-2 gap-2">
              {links.map((link) => (
                <a key={link.label} href={link.href}
                  ...
                  className="flex items-center gap-2.5 px-3 py-2.5 rounded-xl bg-slate-800/40 border border-slate-700/50 text-xs text-slate-400 hover:text-orange-400 hover:border-orange-500/30 hover:bg-slate-800/70 transition-all group">
                  <span className="text-slate-600 group-hover:text-orange-400 transition-colors">{link.icon}</span>
          ...
          <div className="px-5 pb-4">
            <div className="flex items-center gap-3 p-3 rounded-xl bg-slate-800/40 border border-slate-700/50">
              <MessageSquare className="w-5 h-5 text-orange-400/70 flex-shrink-0" />
              <div className="min-w-0">
                <p className="text-xs font-medium text-slate-300">Butuh bantuan lebih lanjut?</p>
                <p className="text-[11px] text-slate-500">
                  Hubungi tim TransitERA di{' '}
                  <a href="mailto:transitera@its.ac.id" className="text-orange-400 hover:underline">transitera@its.ac.id</a>
                </p>
              </div>
            </div>
          </div>
            ...
            <div className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-1.5">
                  {FEEDBACK_CATEGORIES.map((cat) => (
                    <button key={cat.value} onClick={() => setFeedbackCategory(cat.value)}
                      ...
                      <span className={feedbackCategory === cat.value ? 'text-orange-400' : 'text-slate-600'}>{cat.icon}</span>
                      {cat.label}
                    </button>
                  ))}
                </div>
                ...
                <div className="flex items-center gap-1">
                  {[1, 2, 3, 4, 5].map((star) => (
                    <button key={star} onClick={() => setRating(star)}
                      className={`transition-transform hover:scale-110 ${star <= rating ? 'text-amber-400' : 'text-slate-700 hover:text-slate-500'}`}>
              ...
              <div>
                ...
                <textarea value={feedbackText} onChange={(e) => setFeedbackText(e.target.value)}
                  ...
                  rows={4}
                  className="w-full bg-slate-800/50 border border-slate-700 text-xs text-slate-200 placeholder-slate-600 rounded-xl px-4 py-3 resize-none focus:outline-none focus:border-orange-500/50 focus:ring-1 focus:ring-orange-500/20 transition-all" />
              </div>
              ...
              <div>
                ...
                <input type="email" value={feedbackEmail} onChange={(e) => setFeedbackEmail(e.target.value)}
                  ...
                  className="w-full bg-slate-800/50 border border-slate-700 text-xs text-slate-200 placeholder-slate-600 rounded-xl px-4 py-2.5 focus:outline-none focus:border-orange-500/50 focus:ring-1 focus:ring-orange-500/20 transition-all" />
              </div>
